...
void write_bucket_trace_header(std::ofstream& out) {
  out << "filename,bucket_hit_sequence,total_hits,unique_buckets,min_bucket,"
         "max_bucket\n";
}

void write_bucket_trace_row(std::ofstream& out, const std::string& filename,
  ...
  const std::size_t unique_buckets =
      sorted.empty()
          ? 0
          : static_cast<std::size_t>(std::unique(sorted.begin(), sorted.end()) -
                                     sorted.begin());
  ...
  out << "\"," << total_hits << ',' << unique_buckets << ',' << min_bucket
      << ',' << max_bucket << '\n';
}
...
void print_usage(const char* prog) {
  std::cerr << "Usage:\n"
            << "  " << prog
            << " --reference PATH --filenames PATH [--suffix-array PATH] "
...
Args parse_args(int argc, char** argv) {
  ...
  if (!args.bucket_hit_trace.empty() && args.mode == "baseline") {
    throw std::runtime_error(
        "--bucket-hit-trace requires --mode cached or --mode both");
  }
...
int main(int argc, char** argv) {
  try {
    ...
    if (!args.bucket_hit_trace.empty()) {
      ...
      if (!bucket_trace) {
        throw std::runtime_error("cannot open bucket-hit trace file: " +
                                 args.bucket_hit_trace);
      }
    ...
    std::cout << "file,input_bytes,baseline_ms,cached_ms,baseline_phrases,"
                 "cached_phrases,outputs_equal,"
              << "file_cache_hits,file_cache_misses,file_cache_hit_rate,total_"
                 "cache_entries,approx_cache_MB,peak_RSS_MB\n";
    ...
    for (const auto& file : files) {
      ...
      if (access(file.c_str(), R_OK) != 0) {
        std::cerr << "File exists but is not readable: [" << file << "]\n";
        continue;
      }
      ...
      if (args.mode != "baseline") {
        ...
        cached_ms = time_ms([&] { cached = parser->lzFactorize(input); });
        ...
        if (bucket_trace) {
          write_bucket_trace_row(bucket_trace, file,
      ...
      std::cout << file << ',' << input.size() << ',' << std::fixed
                << std::setprecision(3) << baseline_ms << ',' << std::fixed
                << std::setprecision(3) << cached_ms << ',' << baseline_phrases
                << ',' << cached_phrases << ',' << equal_text << ','
                << file_hits << ',' << file_misses << ',' << std::fixed
                << std::setprecision(6) << file_hit_rate << ',' << cache_entries
                << ',' << std::fixed << std::setprecision(3) << cache_mb << ','
                << std::fixed << std::setprecision(2) << peak_rss_kb() / 1024.0
                << '\n';
    }
    ...
    const double avg_cached_ms =
        processed == 0 ? 0.0 : total_cached_ms / static_cast<double>(processed);
    ...
    std::cout << "summary,total_cached_phrases," << total_cached_phrases
              << "\n";
    std::cout << "summary,total_baseline_ms," << std::fixed
              << std::setprecision(3) << total_baseline_ms << "\n";
    std::cout << "summary,total_cached_ms," << std::fixed
              << std::setprecision(3) << total_cached_ms << "\n";
    std::cout << "summary,avg_baseline_ms," << std::fixed
              << std::setprecision(3) << avg_baseline_ms << "\n";
    std::cout << "summary,avg_cached_ms," << std::fixed << std::setprecision(3)
              << avg_cached_ms << "\n";
    std::cout << "summary,overall_speedup," << std::fixed
              << std::setprecision(3) << speedup << "\n";
    std::cout << "summary,all_outputs_equal," << (all_equal ? "YES" : "NO")
              << "\n";
    std::cout << "summary,total_cache_hits," << final_info.hits << "\n";
    std::cout << "summary,total_cache_misses," << final_info.misses << "\n";
    std::cout << "summary,final_cache_hit_rate," << std::fixed
              << std::setprecision(6) << final_info.hit_rate << "\n";
    std::cout << "summary,final_cache_entries," << final_info.current_size
              << "\n";
    std::cout << "summary,final_cache_MB," << std::fixed << std::setprecision(3)
              << final_info.approx_bytes / (1024.0 * 1024.0) << "\n";
    std::cout << "summary,peak_RSS_MB," << std::fixed << std::setprecision(2)
              << peak_rss_kb() / 1024.0 << "\n";
